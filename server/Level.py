...
class Box:
    ...
    def check_collision_line(self, x1: int, y1: int, x2: int, y2: int) -> Collision:
    ...
    @staticmethod
    def _check_collision_line_line(x1: int, y1: int, x2: int, y2: int, x3: int, y3: int, x4: int, y4: int) -> Collision:
        t = ((x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3)) / ((y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1))
        r = ((x2 - x1) * (y1 - y3) - (y2 - y1) * (x1 - x3)) / ((y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1))
        if (0 <= t <= 1) and (0 <= r <= 1):
            return Collision(int(x1 + t * (x2 - x1)), int(y1 + t * (y2 - y1)))
        else:
            return Collision.false()

    @staticmethod
    def _check_collision_line_horizontal(x1: int, y1: int, x2: int, y2: int, x3: int, x4: int, y3: int) -> Collision:
        ...
        if y1 == y2:
            if y1 == y3:
                if x1 >= x3 >= x2 or x2 >= x3 >= x1:
                    return Collision(x3, y3)
                elif x1 >= x4 >= x2 or x2 >= x4 >= x1:
                    return Collision(x4, y3)
            return Collision.false()
        if (y3 < y1) or (y3 > y2):
            return Collision.false()
        t = (y3 - y1) / (y2 - y1)
        ...
        if (x3 <= tx <= x4) or (x4 <= tx <= x3):
            return Collision(tx, y3)
        return Collision.false()

    @staticmethod
    def _check_collision_line_vertical(x1: int, y1: int, x2: int, y2: int, y3: int, y4: int, x3: int) -> Collision:
        """Returns False if no collision, otherwise the collision coordinates (x,y)"""
        if x1 > x2:
            x1, x2 = x2, x1
            y1, y2 = y2, y1
        if x1 == x2:
            if x1 == x3:
                if y1 >= y3 >= y2 or y2 >= y3 >= y1:
                    return Collision(x3, y3)
                elif y1 >= y4 >= y2 or y2 >= y4 >= y1:
                    return Collision(x3, y4)
            return Collision.false()
        if (x3 < x1) or (x3 > x2):
            return Collision.false()
        t = (x3 - x1) / (x2 - x1)
